...
server.tool(
  "mcp-server-to-vscode",
  "Generate a VS Code URI and CLI command to install an MCP server, based on the provided configuration (call multiple times when servers provide different commands, like `docker`, `npx`, or `uvx`). Adapt the response to VS Code stable and insiders, using the referenced badge a button image.",
